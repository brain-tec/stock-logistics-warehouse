...
class TestQuotedQty(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.uom_cat = cls.env["uom.category"].create({"name": "Coolness"})
        cls.uom = cls.env["uom.uom"].create(
            {"name": "odoos", "category_id": cls.uom_cat.id}
        )
        cls.product_tmpl_attr1_v1 = cls.product_tmpl.attribute_line_ids[
            0
        ].product_template_value_ids[0]
        cls.product_tmpl_attr1_v2 = cls.product_tmpl.attribute_line_ids[
            0
        ].product_template_value_ids[1]
        cls.product1 = cls.product_tmpl._get_variant_for_combination(
            cls.product_tmpl_attr1_v1
        )
        cls.product2 = cls.product_tmpl._get_variant_for_combination(
            cls.product_tmpl_attr1_v2
        )
        cls.product1.write(
            {
                "name": "Test variant 1",
        ...
        cls.wh_main = cls.env["stock.warehouse"].create(
            {"name": "Main Test Warehouse", "code": "MTESTW"}
        )
        cls.wh_shop = cls.env["stock.warehouse"].create(
            {"name": "Secondary Test Warehouse", "code": "STESTW"}
        )
    ...
    def assertquotedqty(self, record, qty, msg):
        record.refresh()
        self.assertEqual(record.quoted_qty, qty, msg)

    def test_quoted_qty(self):
        # The quoted quantity should be 0
        self.assertquotedqty(self.product1, 0.0, "Check initial quoted quantity")
        # Enter a Quotation
        ...
        self.assertinmediatelyusableqty(
            self.product1, -107.0, "Check the quantity of variant available for sale"
        )
        self.assertinmediatelyusableqty(
            self.product1.product_tmpl_id,
            -107.0,
        ...
        self.assertquotedqty(
            self.product1.product_tmpl_id, 107.0, "Check quoted quantity for template"
        )
        # Check by warehouse
        self.assertquotedqty(
            self.product1.with_context(warehouse=self.wh_main.id),
            107.0,
            "Check quoted quantity in main WH",
        )
        self.assertquotedqty(
            self.product1.with_context(warehouse=self.wh_shop.id),
            0.0,
            "Check quoted quantity in Chicago WH",
        )
        # Check by location
        self.assertquotedqty(
            self.product1.with_context(location=self.wh_main.lot_stock_id.id),
            107.0,
            "Check quoted quantity in main WH location",
        )
        self.assertquotedqty(
            self.product1.with_context(location=self.wh_shop.lot_stock_id.id),
            0.0,
        ...
        self.assertquotedqty(
            self.product1.product_tmpl_id, 720.0, "Check quoted quantity for template"
        )
        # Check by warehouse
        self.assertquotedqty(
            self.product1.with_context(warehouse=self.wh_main.id),
        ...
        self.assertquotedqty(
            self.product2, 22.0, "Check quoted quantity for second variant"
        )
        self.assertquotedqty(
            self.product1.product_tmpl_id, 742.0, "Check quoted quantity for template"
        )
        ...
        # Check by variant
        self.assertquotedqty(
            self.product1, 613.0, "Check quoted quantity for first variant"
        )
        self.assertquotedqty(
            self.product2, 22.0, "Check quoted quantity for second variant"
        )
        self.assertquotedqty(
            self.product1.product_tmpl_id, 635.0, "Check quoted quantity for template"
        )
        # Check by warehouse
        self.assertquotedqty(
            self.product1.with_context(warehouse=self.wh_main.id),
