...
class ProductProduct(models.Model):
    ...
    def _compute_quoted_qty(self):
        ...
        if self.env.context.get("force_company", False):
            domain.append(("company_id", "=", self.env.context["force_company"]))
        ...
        if self.env.context.get("location", False):
            ...
            # Search by name
                ...
                domain.append(
                    ("order_id.warehouse_id.lot_stock_id", loc_op, location_ids)
                )
        ...
        if self.env.context.get("warehouse", False):
            domain.append(("order_id.warehouse_id", "=", self.env.context["warehouse"]))
        # Limit to a period
        ...
        for group in (
            ...
            results += Counter(
                {
                    product_id: self.env["uom.uom"]
                    .browse(uom_id)
                    ._compute_quantity(
                        group["product_uom_qty"], self.browse(product_id).uom_id
                    )
                }
            )
        for product in self:
